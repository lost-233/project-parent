 ...
 */
@Processor(desc = "test列表V2", service = Service.PLATFORM, method = "zm.test",level = Versions.V2,response = TestResponse.class)
public class TestV2Processor implements ServiceProcessor {

    private final CategoryClient client;

    @Param(desc = "testV2")
    private static final ServiceParameter TEST = getServiceParameter("testV2", false, null);

    @Autowired
    public TestV2Processor(CategoryClient client) {
        this.client = client;
    }
